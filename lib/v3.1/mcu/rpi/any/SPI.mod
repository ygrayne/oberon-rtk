...
**)
  ...
  IMPORT SYSTEM, BASE, DEV := SPI_DEV, RST, Clocks, Errors;
  ...
  BEGIN
    ...
    ASSERT(spiNo IN DEV.SPI_all, Errors.PreCond);
    dev.spiNo := spiNo;
    dev.rstReg := DEV.SPI_RST_reg;
    dev.rstPos := DEV.SPI0_RST_pos + spiNo;
    base := DEV.SPI0_BASE + (spiNo * DEV.SPI_Offset);
    dev.CR0  := base + DEV.SPI_CR0_Offset;
    dev.CR1  := base + DEV.SPI_CR1_Offset;
    dev.CPSR := base + DEV.SPI_CPSR_Offset;
    dev.IMSC := base + DEV.SPI_IMSC_Offset;
    dev.TDR  := base + DEV.SPI_DR_Offset;
    dev.RDR  := base + DEV.SPI_DR_Offset;
    dev.SR   := base + DEV.SPI_SR_Offset
  END Init;
  ...
  **)
  ...
  BEGIN
    ASSERT(dev # NIL, Errors.PreCond);
    SYSTEM.PUT(dev.CR1 + BASE.ASET, {CR1_SSE})
  END Enable;
  ...
  BEGIN
    ASSERT(dev # NIL, Errors.PreCond);
    SYSTEM.PUT(dev.CR1 + BASE.ACLR, {CR1_SSE})
  END Disable;


  PROCEDURE ApplyReset*(dev: Device);
  BEGIN
    RST.ApplyReset(dev.rstReg, dev.rstPos)
  END ApplyReset;
  ...
  BEGIN
    SYSTEM.PUT(dev.CR1 + BASE.ASET, {CR1_LBM})
  END EnableLoopback;


  PROCEDURE* DisableLoopback*(dev: Device);
  BEGIN
    SYSTEM.PUT(dev.CR1 + BASE.ACLR, {CR1_LBM})
  END DisableLoopback;


  (* Secure/Non-secure, RP2350 only *)

  PROCEDURE GetDevSec*(spiNo: INTEGER; VAR reg: INTEGER);
  BEGIN
    ASSERT(spiNo IN DEV.SPI_all);
    reg := DEV.SPI0_SEC_reg + (spiNo * 4)
  END GetDevSec;
