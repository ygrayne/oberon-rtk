...
MODULE Kernel;
  ...
  (* put into ready queue, prio sorted *)
    VAR t0, t1: Thread;
  BEGIN
    ASSERT(~(t.tid IN ctx.queued), Error.PreCond);
    t0 := ctx.ct; t1 := t0;
  ...
  PROCEDURE loopc;
    VAR tid, cid: INTEGER; t: Thread; ctx: CoreContext;
  BEGIN
    SYSTEM.GET(MCU.SIO_CPUID, cid);
    ctx := coreCon[cid];
    ctx.Ct := NIL;
      ...
      IF SysTick.Tick() THEN
        ...
        WHILE tid < ctx.numThreads DO
          ...
          IF t.state = StateEnabled THEN
            IF (t.delay <= 0) & (t.period = 0) THEN (* no triggers *)
            ...
            ELSE
              ...
              IF t.period > 0 THEN (* thread periodically triggered *)
                ...
                IF t.ticker <= 0 THEN
                  ...
                  IF t.delay <= 0 THEN (* delay takes precedence *)
                    IF ~(t.tid IN ctx.queued) THEN
                      slotIn(t, ctx)
                    END
                  END
