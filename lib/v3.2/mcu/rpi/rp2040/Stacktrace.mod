...
(**
  ...
  MCU: RP2040
  --
  Note: this is ported from the M33 implementation with as few as possible
  changes; hence, it covers more cases than found on the M0+, but the logic
  is sufficiently complex to suffer some unused code for the benefit
  of correctness.
  --
...
**)
  ...
  IMPORT SYSTEM, ProgData, RuntimeErrors, MemMap;
  ...
  (* note: excRetVal: unused on M0+ (no FPU); kept for signature parity with M33 *)
