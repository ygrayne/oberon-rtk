...
**)
  ...
  IMPORT SYSTEM, BASE, RST, SYS := CLK_SYS, Errors;
  ...
  PROCEDURE* StartXOSC*;
  (* 12 MHz *)
    CONST XOSC_Enable  = 0FABH; XOSC_STATUS_STABLE = 31;
    VAR val: INTEGER;
  BEGIN
  ...
  PROCEDURE AttachPLL*(pll: INTEGER);
  BEGIN
    ASSERT(pll IN SYS.PLL_all, Errors.PreCond);
    IF pll = PLLsys THEN
  ...
  BEGIN
    ...
    SYSTEM.PUT(SYS.CLK_SYS_DIV, val);
    ...
    SYSTEM.PUT(SYS.CLK_REF_DIV, val);
  ...
  (* applies to watchdog, systick, timer *)
    CONST WATCHDOG_TICK_EN  = 9;
  BEGIN
    SYSTEM.PUT(SYS.CLK_WATCHDOG_TICK, cfg.watchdogDiv);
    SYSTEM.PUT(SYS.CLK_WATCHDOG_TICK + BASE.ASET, {WATCHDOG_TICK_EN})
  END ConfigTicks;


  PROCEDURE* GetTicksCfg*(VAR cfg: TicksCfg);
  BEGIN
    SYSTEM.GET(SYS.CLK_WATCHDOG_TICK, cfg.watchdogDiv)
  END GetTicksCfg;
