...
def _topo_sort(sources):
    ...
    if len(order) != len(sources):
        ...
        print(f'merge-defs: circular dependency among: {", ".join(sorted(missing))}')
        sys.exit(1)
...
def generate_module(name, sources):
    ...
    out.append(f'(** Generated by merge-defs. Do not edit. **)')
    out.append('')
...
def main():
    parser = argparse.ArgumentParser(
        prog='merge-defs',
        description='Merge per-device definition modules into consolidated modules.',
    )
    ...
    if not args.source_dir.is_dir():
        print(f'merge-defs: source directory not found: {args.source_dir}')
        sys.exit(1)

    if not args.output_dir.is_dir():
        print(f'merge-defs: output directory not found: {args.output_dir}')
        sys.exit(1)
    ...
    if not groups:
        print(f'merge-defs: no *_SYS.mod or *_DEV.mod files in {args.source_dir}')
        sys.exit(1)
    ...
    for out_name, source_files in groups.items():
        ...
        for sf in source_files:
            ...
            if name is None:
                print(f'merge-defs: cannot parse MODULE name in {sf}')
                sys.exit(1)
            if not const_lines:
                if args.verbose:
        ...
        print(f'merge-defs: {output_path} ({len(sources)} sources)')
