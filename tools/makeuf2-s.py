...
class RPcmd(commands.Command):
    ...
    def _run(self, args):
        file_re = re.compile(self._file_pat)
        print("file", args.bin_file)
        m = file_re.match(args.bin_file)
        print("m", m)
        if m is None:
            print(f'{PROG_NAME}: specify binary as \'bin_file:load_address\'')
            sys.exit(1)
        else:
            try:
            ...
            bin_file = Path(m.group(1)).resolve()
            if not bin_file.is_file():
                print(f'{PROG_NAME}: cannot find binary file {bin_file}')
                sys.exit(1)
        ...
        try:
            ...
            sys.exit(1)
        self._max_addr = XIP_BASE + flash_size
    ...
    def _print_verbose(self):
        print(f'(secure) image file:            {self._bin_file.file} addr: {self._img_addr:x}')
        if hasattr(self, '_boot2_file'):
            print(f'boot2 image file:           {self._boot2_file.file}')
        if hasattr(self, '_ns_bin_file'):
            print(f'non-secure image file:          {self._ns_bin_file.file} addr: {self._ns_img_addr:x}')
        if hasattr(self, '_nsc_bin_file'):
            print(f'non-secure callable image file: {self._nsc_bin_file.file} addr: {self._nsc_img_addr:x}')
        ...
        print("program memory range: 0x{:x} 0x{:x}".format(self._img_addr, self._max_addr))
...
class RP2040(RPcmd):
    ...
    def run(self, args):
        ...
        self._uf2_file.add_prog_blocks(self._bin_file.data, self._bin_file.num_blocks() + 1, UF2_FAMID_RP2040, self._img_addr, self._max_addr)
...
class RP2350(RPcmd):
    ...
    _defs = {
        ...
        'oargs': {
            'nonsec': {
                'flags': ['--ns'],
                'help': 'create a non-secure image',
                'action': 'store_true'
            },
            'drive': {
                'flags': ['-d'],
    ...
    def run(self, args):
        ...
        meta_addr = self._load_addr
        vtab_addr = meta_addr + UF2_IMAGE_DEF_SIZE
        num_blocks = self._bin_file.num_blocks() + 1
        if args.nonsec:
            self._uf2_file.add_meta_block(num_blocks, UF2_FAMID_RP2350_ARM_NS, META_IMAGE_DEF_NS, meta_addr, vtab_addr)
            self._uf2_file.add_prog_blocks(self._bin_file.data, num_blocks, UF2_FAMID_RP2350_ARM_NS, vtab_addr, self._max_addr)
        else:
            self._uf2_file.add_meta_block(num_blocks, UF2_FAMID_RP2350_ARM_S, META_IMAGE_DEF_S, meta_addr, vtab_addr)
            self._uf2_file.add_prog_blocks(self._bin_file.data, num_blocks, UF2_FAMID_RP2350_ARM_S, vtab_addr, self._max_addr)
        self._uf2_file.close()
        print("UF2 binary {} created.".format(self._uf2_file.file.name))
...
class UF2file:
    ...
    def add_meta_block(self, num_blocks, fam_id, img_def, meta_addr, vtab_addr):
        # print(hex(img_def))
        # try:
        self._fd.write(UF2_BLOCK_BEGIN)
        self._fd.write(UF2_MAGIC_BEGIN)
        self._fd.write(UF2_FLAGS)
        self._fd.write(struct.pack("<I", meta_addr))
        ...
        # self._fd.write(META_ROLL_WIN_DELTA)
        # self._fd.write(struct.pack("<i", rw_delta))
