MODULE RTCds3234test;
(**
  Oberon RTK Framework
  --
  SPI driver for Maxim DS3234 real-time clock
  Test version for https://oberon-rtk.org/docs/examples/v2/spirtc2/
  --
...
**)
  ...
  (* normally we could just use GPIO.Set({cd}) *)
  ...
  BEGIN
    ...
    cs := csPinNo;
    (* test variations *)
    SPIdev.GetBaseRunCfg(spiDev, rtc0);
    rtc1 := rtc0;
    SPIdev.SetTxShift(rtc0, 0AAH);
    SPIdev.SetSclkRate(spiDev, rtc1, 2000000)
  END Install;
