...
**)
  ...
  IMPORT
    SYSTEM, Main, MCU := MCU2, Kernel, GPIO, SIO := SIOgpio, Exceptions, Out, Errors;

  CONST
    HighPrio = MCU.PPB_ExcPrioTop;
    MedPrio = MCU.PPB_ExcPrioMedium;
    LowPrio = MCU.PPB_ExcPrioLow;
    SysTickPrio = MCU.PPB_ExcPrioHigh;

    HighRunIntNo = MCU.PPB_SPAREIRQ_IRQ0;
    MedRunIntNo = MCU.PPB_SPAREIRQ_IRQ1;
    LowRunIntNo = MCU.PPB_SPAREIRQ_IRQ2;
...
*)
  ...
  PROCEDURE run;
    CONST GPIOintNo = MCU.PPB_IO_IRQ_BANK0; (*TRCENA = 24; CYCCNTENA = 0;*)
    (*VAR s: SET;*)
  BEGIN
    ...
    SIO.OutputEnable({Pin1, Pin2, Pin3, Pin4});
    SIO.Clear({Pin1, Pin2, Pin3, Pin4});
