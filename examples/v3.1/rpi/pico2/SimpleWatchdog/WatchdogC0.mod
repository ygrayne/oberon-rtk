...
**)
  ...
  IMPORT
    SYSTEM, BASE, Main, Kernel, Out, Cores, RST, Errors,
    LED, Watchdog, CoreOne := WatchdogC1, WATCHDOG_DEV, RESETS_SYS;
  ...
  BEGIN
    IF SYSTEM.BIT(WATCHDOG_DEV.WATCHDOG_SCRATCH0, Core1FailFlag) THEN
  ...
  BEGIN
    IF core1Fail THEN
      SYSTEM.PUT(WATCHDOG_DEV.WATCHDOG_SCRATCH0 + BASE.ACLR, {Core1FailFlag})
    ELSE
      SYSTEM.PUT(WATCHDOG_DEV.WATCHDOG_SCRATCH0 + BASE.ASET, {Core1FailFlag})
    END
  END switchTestCase;
  ...
  BEGIN
    ...
    core1missing := 0;
    REPEAT
      IF SYSTEM.BIT(WATCHDOG_DEV.WATCHDOG_SCRATCH0, Core1OkFlag) THEN
        core1missing := 0;
        SYSTEM.PUT(WATCHDOG_DEV.WATCHDOG_SCRATCH0 + BASE.ACLR, {Core1OkFlag})
      ELSE
        INC(core1missing)
      END;
  ...
  PROCEDURE run;
    VAR res: INTEGER;
  BEGIN
    Cores.StartCoreOne(CoreOne.Run, Main.ConfigC1);
    ...
    RST.SetPowerOnWatchdogResets(RESETS_SYS.PSM_RESET);
    Watchdog.Enable;
