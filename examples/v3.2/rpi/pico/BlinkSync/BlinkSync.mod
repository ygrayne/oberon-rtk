...
(**
  ...
  --
  Core 0 program: example program BlinkPlus extended to start core 1 program
  Core 1 program: adapted SignalSync
  --
  MCU: RP2040
  Board: Pico
  --
...
**)
  ...
  IMPORT Main, Kernel, Out, Cores, Errors, LED, TIMER, SignalSync;
  ...
  BEGIN
    ...
    Kernel.Allocate(perodic, ThreadStackSize, t1, tid1, res);
    ASSERT(res = Kernel.OK, Errors.ProgError);
    Kernel.Enable(t1);
    Kernel.Allocate(perodic, ThreadStackSize, t2, tid2, res);
    ASSERT(res = Kernel.OK, Errors.ProgError);
    Kernel.Enable(t2);
