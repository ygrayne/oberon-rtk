...
**)
  ...
  CONST
    ...
    DEBUG = TRUE; (* Astrobe v10 will not compile/include this code *)
  ...
  BEGIN
    ...
    x := 0;
    IF DEBUG THEN (* allow terminal output and LEDs *)
      WHILE x < 1000000 DO INC(x) END
    END;
    Watchdog.Trigger
  END WatchdogFail;
  ...
  PROCEDURE errorHandler[0];
    CONST R12 = 12;
    VAR cid, sl, scrRestart, x: INTEGER;
  BEGIN
    REPEAT
