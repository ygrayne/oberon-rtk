...
**)
  ...
  IMPORT SYSTEM, Coroutines, Config, Memory, SysTick, MCU := MCU2, Errors, GPIO;
  ...
  (* put into ready queue, prio sorted *)
    VAR t0, t1: Thread;
  BEGIN
    SYSTEM.PUT(MCU.SIO_GPIO_OUT_SET, {SlotInPinNo});
    ...
    END;
    SYSTEM.PUT(MCU.SIO_GPIO_OUT_CLR, {SlotInPinNo})
  END slotIn;
  ...
  BEGIN
    ...
    REPEAT
      IF SysTick.Tick() THEN
        SYSTEM.PUT(MCU.SIO_GPIO_OUT_SET, {LoopRunningPinNo});
        tid := 0;
        WHILE tid < ctx.numThreads DO
          ...
          INC(tid)
        END;
        SYSTEM.PUT(MCU.SIO_GPIO_OUT_CLR, {LoopRunningPinNo});
      END;
      WHILE ctx.ct # NIL DO
        t := ctx.ct;
