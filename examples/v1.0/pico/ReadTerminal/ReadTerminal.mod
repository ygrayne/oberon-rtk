...
**)
  ...
  BEGIN
    ...
    ELSIF res = In.NoInput THEN
      Out.String("no input")
    ELSIF res = In.FifoOverrun THEN
      Out.String("FIFO overrun")
    END
  END writeReadError;
  ...
  PROCEDURE t1c;
    VAR str: ARRAY 48 OF CHAR; res, int: INTEGER; (* small buffer to test string overflows *)
  BEGIN
    REPEAT
      REPEAT
        Out.String("enter string: ");
        In.String(str, res);
        IF res = In.FifoOverrun THEN
          writeReadError(res); Out.String(", string truncated"); Out.Ln;
  ...
  BEGIN
    ...
    Kernel.SetPeriod(t1, 100, 0); Kernel.Enable(t1);
    Kernel.Run
